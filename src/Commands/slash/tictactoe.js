...
module.exports = {
	name: "tictactoe",
	description: "Chơi TicTacToe với bạn của bạn",
	options: [
		{
			name: "user",
			type: 6,
	...
	async run(client, interaction, option) {
		var player1 = interaction.user;
		var player2 = option[0].user;
		if (player1.id === player2.id) return interaction.reply({content: `TicTacToe | **${player1} -VS- ${player2}** và cuối cùng ${player1} thắng...?!?`, ephemeral:true})
		if (player1.bot || player2.bot) return interaction.reply({content: `Ban định chơi với máy sao?`, ephemeral:true})
        ...
        const Tie = () => {
            embed 
                ...
                .setTimestamp();
            interaction.editReply({embeds: [embed], components: [hang1, hang2, hang3]});
        }
		...
		async function nextTurn() {
			...
			embed
				...
				.setFooter({ text: `Đến lượt ${player.username}` })
				.setTimestamp();
			await interaction.editReply({ embeds: [embed], components: [hang1, hang2, hang3] });
			Runner();
		};
		Runner();
	},
