...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        if (!args[0]) return message.channel.send(`${client.emotes.filter} Filter : **${queue.filters.length === 0 ? "Tắt" : queue.filters}**`)
        else if (args[0].toLowerCase() === "off" && queue.filters?.length) {
            queue.setFilter(false)
            message.channel.send(`${client.emotes.filter} | Tắt toàn bộ Filter \n*Vui lòng đợi một lát để áp dụng thay đổi*`)
        }
            ...
            message.channel.send(`${client.emotes.filter} | ${args[0]} : ${status} \n*Vui lòng đợi một lát để áp dụng thay đổi*`);
        }
        else if (args[0]) return message.channel.send(`${client.emotes.error} | Không rõ Filter`)
    }
}